...
WebUI.click(findTestObject('trf/trf/other acc/Page_Maybank2u  Maybank Malaysia/div_other maybank name',[('id') :findTestData('Transfer').getValue(2,42)]))
...
for (u = 1; u <= 4; u++) {
	...
	WebUI.setText(findTestObject('trf/other acc-not mbb/Page_Maybank2u  Maybank Malaysia/input_Account number_beneficiaryAccountNumber'),
		findTestData('Transfer').getValue(u+1,44))
	...
	WebUI.setText(findTestObject('trf/other acc-not mbb/Page_Maybank2u  Maybank Malaysia/input_concat(Recipient  s Reference)_transactionReferenceNo'),
		findTestData('Transfer').getValue(u+1,47))
	...
	WebUI.callTestCase(findTestCase('TAC'), [('TACmessage'): findTestData('Transfer').getValue(2,48)])
